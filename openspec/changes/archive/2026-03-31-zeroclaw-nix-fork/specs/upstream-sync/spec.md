...
### Requirement: Sync checklist documented in CONTRIBUTING.md
The fork SHALL include a `CONTRIBUTING.md` (or `SYNC.md`) documenting the step-by-step upstream sync process: fetch, merge tag, rebase sid, update nix hashes, build, test, push. This document SHALL be the authoritative reference for performing syncs.
