...
Generic, reusable NixOS module for ZeroClaw in the `kcalvelli/zeroclaw-nix` fork. Provides typed options, freeform settings rendered to TOML, declarative channel configuration with secret file injection, and systemd hardening.
...
- **WHEN** `services.zeroclaw.channels.telegram = { enable = true; botTokenFile = "/run/agenix/telegram-token"; allowed_users = [ "12345" ]; }`
...
- **WHEN** `services.zeroclaw.channels.email = { enable = true; passwordFile = "/run/agenix/email-password"; imap_host = "london.mxroute.com"; username = "genxbot@calvelli.us"; }`
...
The module SHALL apply systemd security hardening: `ProtectHome`, `ProtectSystem=strict`, `PrivateTmp`, `NoNewPrivileges`, `CapabilityBoundingSet=""`, restricted address families, and `SystemCallFilter=@system-service`. The state directory SHALL be the only writable path.

#### Scenario: Hardened service unit
- **WHEN** inspecting the generated systemd unit
- **THEN** all hardening directives SHALL be present
...
- **WHEN** running `nix build github:kcalvelli/zeroclaw-nix`
- **THEN** the output SHALL be the zeroclaw binary with all feature patches applied
